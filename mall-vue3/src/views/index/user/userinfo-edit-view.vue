...
const submit =()=> {
  ...
  updateUserInfoApi(formData).then(res => {
    ...
    // 更新用户store中的昵称
    if (tData.form.nickname) {
      userStore.user_nickname = tData.form.nickname
      localStorage.setItem('user_nickname', tData.form.nickname)
    }
    
    // 如果有头像更新，重新获取头像信息
    if (tData.form.avatarFile) {
      // 在getUserInfo中会重新获取头像路径，之后需要更新store
      setTimeout(() => {
        if (tData.form.avatar) {
          const avatarName = tData.form.avatar.split('/').pop()
          userStore.user_avatar = avatarName
